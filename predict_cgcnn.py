...
#data__.to_csv(f"results/{metal.split('-')[1]}_impure.csv")

r2 = RegressionMetrics(data__['true'], data__['pred']).r2()
rmse = RegressionMetrics(data__['true'], data__['pred']).rmse()
mae = RegressionMetrics(data__['true'], data__['pred']).mae()
...
annotate(ax, f'$R^2$: {round(r2, 2)}', xy=(0.1, 0.95))
annotate(ax, f'RMSE: {round(rmse, 2)}', xy=(0.1, 0.85))
annotate(ax, f'MAE: {round(mae, 2)}', xy=(0.1, 0.75))
